...
public class AvoidOnAxis : NetworkBehaviour
{
    [System.Serializable]
    ...
    public void ActivateBehaviour(AvoidParameters avoidParams)
    {
        AvoidParams = avoidParams;
        ActivateBehaviour();
    }
    ...
    public void ActivateBehaviour()
    {
        ...
        {
            //Add our variation our speed and distance
        ...
        else if (m_State == State.Returning)
        {
            m_State = State.Avoiding;
            m_TimeNeeded = m_DistanceWithVariaton / m_SpeedWithVariaton;
            m_TimeProgress = ((transform.position - m_StartPosition).magnitude / m_DistanceWithVariaton) * m_TimeNeeded;
            RpcActivate(m_StartPosition, m_TargetPosition, m_TimeProgress, m_TimeNeeded, m_State);
        }
    }
    ...
    private void Update()
    {
        if (m_State == State.Avoiding)
        {
            ...
            m_Collider.center -= (lerpTarget - transform.position);
